...
// TipKit is only available on iOS 17+
#if canImport(TipKit)
import TipKit
#endif
...
final class MyRoomsViewController: UIViewController {
    ...
    // TipKit (iOS 17+) - used for lightweight UI discovery only
    private var scanButtonForTip: UIButton?
    private var tipObservationTask: Any?
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // Attach TipKit tip after the navigation bar is visible
        setupScanRoomTip()
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        // Clean up TipKit observation when leaving the view
        #if canImport(TipKit)
        if #available(iOS 17.0, *) {
            if let task = tipObservationTask as? Task<Void, Never> {
                task.cancel()
            }
    ...
    private func setupEmptyState() {
        ...
        titleLabel.text = "No Rooms Scanned Yet"
        ...
        messageLabel.text = "Tap the camera icon to scan your room\nusing your device's LiDAR sensor,\nor import existing USDZ files."
        ...
        tipsContainer.backgroundColor = .systemGray6
        tipsContainer.layer.cornerRadius = 12
        
        let tipsLabel = UILabel()
        tipsLabel.text = "💡 Tips for Best Results"
        tipsLabel.font = .boldSystemFont(ofSize: 14)
        tipsLabel.textColor = .label
        tipsLabel.translatesAutoresizingMaskIntoConstraints = false
        
        let tipsList = UILabel()
        tipsList.text = "• Ensure good lighting in the room\n• Move slowly and steadily\n• Cover all corners and walls\n• Keep 2-3 feet from surfaces"
        tipsList.font = .systemFont(ofSize: 13)
        tipsList.textColor = .secondaryLabel
        tipsList.numberOfLines = 0
        tipsList.translatesAutoresizingMaskIntoConstraints = false
        
        tipsContainer.addSubview(tipsLabel)
        tipsContainer.addSubview(tipsList)
        
        emptyStateView.addSubview(iconView)
        emptyStateView.addSubview(titleLabel)
        emptyStateView.addSubview(messageLabel)
        emptyStateView.addSubview(tipsContainer)
        ...
        NSLayoutConstraint.activate([
            ...
            tipsContainer.topAnchor.constraint(equalTo: messageLabel.bottomAnchor, constant: 24),
            tipsContainer.leadingAnchor.constraint(equalTo: emptyStateView.leadingAnchor),
            tipsContainer.trailingAnchor.constraint(equalTo: emptyStateView.trailingAnchor),
            tipsContainer.bottomAnchor.constraint(equalTo: emptyStateView.bottomAnchor),
            
            tipsLabel.topAnchor.constraint(equalTo: tipsContainer.topAnchor, constant: 12),
            tipsLabel.leadingAnchor.constraint(equalTo: tipsContainer.leadingAnchor, constant: 16),
            tipsLabel.trailingAnchor.constraint(equalTo: tipsContainer.trailingAnchor, constant: -16),
            
            tipsList.topAnchor.constraint(equalTo: tipsLabel.bottomAnchor, constant: 8),
            tipsList.leadingAnchor.constraint(equalTo: tipsContainer.leadingAnchor, constant: 16),
            tipsList.trailingAnchor.constraint(equalTo: tipsContainer.trailingAnchor, constant: -16),
            tipsList.bottomAnchor.constraint(equalTo: tipsContainer.bottomAnchor, constant: -12)
        ])
    }
    
    private func updateEmptyState() {
        emptyStateView.isHidden = !roomFiles.isEmpty
        collectionView.isHidden = roomFiles.isEmpty
    }
    
    // MARK: - TipKit Setup
    /// Sets up the TipKit tip for the scan room button.
    /// TipKit is only used for lightweight UI discovery (buttons, filters).
    /// It is NOT used in AR views, RoomPlan scanning, or Object Capture cameras.
    private func setupScanRoomTip() {
        #if canImport(TipKit)
        if #available(iOS 17.0, *) {
            let tip = ScanRoomTip()
            
            // Observe the tip status and attach popover when eligible
            tipObservationTask = Task { @MainActor in
                for await shouldDisplay in tip.shouldDisplayUpdates {
                    if shouldDisplay {
                        let tipView = TipUIView(tip) { [weak self] _ in
                            // User tapped the close button - invalidate the tip
                            tip.invalidate(reason: .actionPerformed)
                        }
                        tipView.translatesAutoresizingMaskIntoConstraints = false
                        tipView.tag = 999  // Tag for easy removal
                        
                        // Create a popover-style container
                        ...
                        NSLayoutConstraint.activate([
                            tipView.topAnchor.constraint(equalTo: self.view.safeAreaLayoutGuide.topAnchor, constant: 8),
                            tipView.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -16),
                            tipView.widthAnchor.constraint(lessThanOrEqualToConstant: 280)
                        ])
                    } else {
                        // Tip should no longer display - remove it from view
                        if let existingTipView = self.view.viewWithTag(999) {
                            existingTipView.removeFromSuperview()
                        }
                    }
                }
            }
